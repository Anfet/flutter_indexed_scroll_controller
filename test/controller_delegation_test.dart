...
void main() {
  group('IndexedScrollController delegation tests (pre-extends)', () {
    group('onAttach/onDetach callbacks', () {
      testWidgets('onAttach is called when controller attaches to ScrollPosition', (WidgetTester tester) async {
        ...
        expect(attachCalls.length, equals(1), reason: 'onAttach should be called exactly once when ListView attaches controller');
        expect(attachCalls[0], isA<ScrollPosition>(), reason: 'onAttach should receive a ScrollPosition');
      });

      testWidgets('onDetach is called when controller detaches from ScrollPosition', (WidgetTester tester) async {
        ...
        expect(detachCalls.length, equals(1), reason: 'onDetach should be called exactly once when ListView is unmounted');
        expect(detachCalls[0], isA<ScrollPosition>(), reason: 'onDetach should receive a ScrollPosition');
      });

      testWidgets('onAttach and onDetach track attachment lifecycle correctly', (WidgetTester tester) async {
        ...
        await tester.pumpAndSettle();
        expect(calls, equals(['attach']), reason: 'Should have one attach call after initial mount');
        ...
        await tester.pumpAndSettle();
        expect(calls, equals(['attach', 'detach']), reason: 'Should have attach then detach after lifecycle');
      });
    });

    group('addListener and removeListener', () {
      testWidgets('addListener: registered listener is called on scroll', (WidgetTester tester) async {
        ...
        expect(changeCount[0], equals(0), reason: 'Listener should not be called before any scroll');

        // Perform a jumpTo
        state.controller.jumpTo(100.0);
        await tester.pump();

        expect(changeCount[0], greaterThan(0), reason: 'Listener should be called after jumpTo changes the scroll position');
      });

      testWidgets('removeListener: removed listener is not called on scroll', (WidgetTester tester) async {
        ...
        expect(changeCount[0], equals(countAfterFirstScroll), reason: 'Removed listener should not be called after removeListener');
      });

      testWidgets('multiple listeners are all called independently', (WidgetTester tester) async {
        ...
        expect(calls1[0], greaterThan(0), reason: 'First listener should be called');
        expect(calls2[0], greaterThan(0), reason: 'Second listener should be called');
        expect(calls1[0], equals(calls2[0]), reason: 'Both listeners should be called the same number of times');
      });
    });

    group('offset property and keepScrollOffset', () {
      testWidgets('offset property returns current scroll position', (WidgetTester tester) async {
        ...
        // Initially at top
        expect(state.controller.offset, equals(0.0), reason: 'Offset should be 0 at the top of the list');

        // Jump to a position
        state.controller.jumpTo(250.0);
        await tester.pump();

        expect(state.controller.offset, equals(250.0), reason: 'Offset should match the position after jumpTo');
      });

      testWidgets('initialScrollOffset is preserved through controller lifecycle', (WidgetTester tester) async {
        ...
        expect(controller.initialScrollOffset, equals(initialOffset), reason: 'initialScrollOffset should be preserved');
      });

      testWidgets('keepScrollOffset affects offset restoration when ScrollPosition is recreated', (WidgetTester tester) async {
        ...
        expect(controller.keepScrollOffset, isTrue, reason: 'keepScrollOffset should be accessible via delegated property');
      });
    });

    group('jumpTo and animateTo delegation', () {
      testWidgets('jumpTo moves scroll position immediately without animation', (WidgetTester tester) async {
        ...
        expect(state.controller.offset, equals(300.0), reason: 'jumpTo should set offset immediately');
      });

      testWidgets('animateTo moves scroll position with animation', (WidgetTester tester) async {
        ...
        expect(midOffset, lessThan(400.0), reason: 'After first pump, offset should be less than target');

        // Complete the animation
        await tester.pumpAndSettle();

        expect(state.controller.offset, equals(400.0), reason: 'animateTo should reach target after animation completes');

        // The future should complete successfully
        await expectLater(future, completes);
      });

      testWidgets('jumpTo and animateTo work correctly individually', (WidgetTester tester) async {
    ...
    group('dispose behavior', () {
      testWidgets('dispose releases resources without throwing', (WidgetTester tester) async {
      ...
      testWidgets('listeners are cleaned up after dispose and do not fire after disposal', (WidgetTester tester) async {
        ...
        expect(
          ...
          reason: 'After dispose, position should be inaccessible (throws AssertionError)',
        );
      });

      testWidgets('multiple calls to dispose throw on second call', (WidgetTester tester) async {
        ...
        expect(
          () => controller.dispose(),
          throwsFlutterError,
          reason: 'Second dispose throws FlutterError (A ScrollController was used after being disposed)',
        );
      });
    });

    group('offset persistence and attachment to multiple lists', () {
      testWidgets('offset is reset to 0 when ListView is remounted (current behavior)', (WidgetTester tester) async {
        ...
        expect(controller.offset, equals(0.0), reason: 'Current behavior: offset resets to 0 when ListView is remounted (ISC-17 may fix this)');
      });

      testWidgets('hasClients reflects whether controller has active positions', (WidgetTester tester) async {
        ...
        expect(controller.hasClients, isFalse, reason: 'hasClients should be false before ListView is mounted');
        ...
        expect(controller.hasClients, isTrue, reason: 'hasClients should be true while ListView is mounted');
        ...
        expect(controller.hasClients, isFalse, reason: 'hasClients should be false after ListView is unmounted');
      });
    });

    group('position and positions properties', () {
      testWidgets('position property delegates to inner controller', (WidgetTester tester) async {
        ...
        expect(controller.position, isA<ScrollPosition>(), reason: 'position should return a ScrollPosition when attached');

        expect(controller.position.pixels, equals(0.0), reason: 'position.pixels should reflect current scroll offset');
      });

      testWidgets('positions property returns iterable of attached positions', (WidgetTester tester) async {
        ...
        expect(positions.length, equals(1), reason: 'Normally a controller has exactly one position (single ListView)');
        expect(positions[0], isA<ScrollPosition>());
      });
    });

    group('debugLabel and debug properties', () {
      testWidgets('debugLabel is accessible via delegated property', (WidgetTester tester) async {
    ...
    group('listener management with measurement operations', () {
      testWidgets('listeners receive updates during scrollTo operations', (WidgetTester tester) async {
        ...
        expect(updateCounts[0], greaterThan(initialCount), reason: 'Listeners should be called during scrollTo animation frames');
      });
    });
  });
}
...
class _ControllerAttachTestWidget extends StatefulWidget {
  ...
  @override
  State<_ControllerAttachTestWidget> createState() => _ControllerAttachTestWidgetState();
}

class _ControllerAttachTestWidgetState extends State<_ControllerAttachTestWidget> {
